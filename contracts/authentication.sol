...
//Don't worry about gas prices as it will be entirely based on test network
//But make sure that even if doesn't cost money, considering running time, minimum number of transactions should be made
contract Interoperability_Mechanism {

    event get_request(string filterValue,string HTTPObj);
    event get_response(string filterValue,string HTTPObj);


    function request_handler(string memory filterValue,string memory HTTPObj) public {
        emit get_request(filterValue,HTTPObj);
    }

    
    function response_handler(string memory filterValue,string memory HTTPObj) public{
        emit get_response(filterValue,HTTPObj);
    }
